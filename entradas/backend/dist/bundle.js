...
__export(db_relations_exports, {
  itensNfRelations: () => itensNfRelations,
  notasFiscaisRelations: () => notasFiscaisRelations
});
import { relations } from "drizzle-orm/relations";
var itensNfRelations = relations(itensNf, ({ one }) => ({
  notasFiscai: one(notasFiscais, {
    fields: [itensNf.notaFiscalId],
    references: [notasFiscais.id]
  })
}));
var notasFiscaisRelations = relations(notasFiscais, ({ many }) => ({
  itensNfs: many(itensNf)
}));
...
async function createApp(edgespark) {
  const app = new Hono2();
  const { fornecedores: fornecedores2, notasFiscais: notasFiscais2, itensNf: itensNf2, itensCatalogo: itensCatalogo2 } = db_schema_exports;
  ...
  app.get("/api/fornecedores", async (c) => {
    ...
    const result = await db.select().from(fornecedores2).where(eq(fornecedores2.userId, user().id)).orderBy(desc(fornecedores2.createdAt));
    return c.json({ data: result });
  });
  app.post("/api/fornecedores", async (c) => {
    const body = await c.req.json();
    console.log("[API] POST /api/fornecedores - creating:", body.nome);
    const result = await db.insert(fornecedores2).values({
      nome: body.nome,
      cnpj: body.cnpj || null,
  ...
  app.put("/api/fornecedores/:id", async (c) => {
    ...
    const result = await db.update(fornecedores2).set({
      nome: body.nome,
      cnpj: body.cnpj || null,
      telefone: body.telefone || null,
      email: body.email || null,
      endereco: body.endereco || null,
      observacoes: body.observacoes || null
    }).where(and(eq(fornecedores2.id, id), eq(fornecedores2.userId, user().id))).returning();
    if (result.length === 0) {
      return c.json({ error: "Fornecedor n\xE3o encontrado" }, 404);
    }
  ...
  app.delete("/api/fornecedores/:id", async (c) => {
    ...
    const result = await db.delete(fornecedores2).where(and(eq(fornecedores2.id, id), eq(fornecedores2.userId, user().id))).returning();
    if (result.length === 0) {
      return c.json({ error: "Fornecedor n\xE3o encontrado" }, 404);
    }
  ...
  app.get("/api/notas-fiscais", async (c) => {
    ...
    let query = db.select().from(notasFiscais2).where(eq(notasFiscais2.userId, user().id));
    const conditions = [eq(notasFiscais2.userId, user().id)];
    if (dataInicio) {
      conditions.push(gte(notasFiscais2.dataEntrada, dataInicio));
    }
    if (dataFim) {
      conditions.push(lte(notasFiscais2.dataEntrada, dataFim));
    }
    if (fornecedorFilter) {
      conditions.push(like(notasFiscais2.fornecedor, `%${fornecedorFilter}%`));
    }
    const result = await db.select().from(notasFiscais2).where(and(...conditions)).orderBy(desc(notasFiscais2.dataEntrada));
    const notasComItens = await Promise.all(
      result.map(async (nota) => {
        const itens = await db.select().from(itensNf2).where(eq(itensNf2.notaFiscalId, nota.id));
        const valorTotal = itens.reduce((sum, item) => {
          const qtd = Number(item.quantidade) || 0;
          const preco = Number(item.precoUnitario) || 0;
          const desc2 = Number(item.desconto) || 0;
          return sum + qtd * preco - desc2;
        }, 0);
        return {
          ...nota,
          itens,
  ...
  app.get("/api/notas-fiscais/:id", async (c) => {
    ...
    const result = await db.select().from(notasFiscais2).where(and(eq(notasFiscais2.id, id), eq(notasFiscais2.userId, user().id)));
    if (result.length === 0) {
      return c.json({ error: "Nota fiscal n\xE3o encontrada" }, 404);
    }
    const nota = result[0];
    const itens = await db.select().from(itensNf2).where(eq(itensNf2.notaFiscalId, nota.id));
    const valorTotal = itens.reduce((sum, item) => {
  ...
  app.post("/api/notas-fiscais", async (c) => {
    ...
    const result = await db.insert(notasFiscais2).values({
      numero: body.numero,
  ...
  app.put("/api/notas-fiscais/:id", async (c) => {
    ...
    const result = await db.update(notasFiscais2).set({
      numero: body.numero,
      dataEmissao: body.dataEmissao,
      dataEntrada: body.dataEntrada,
      fornecedor: body.fornecedor,
      ...
      observacoes: body.observacoes || null,
      status: body.status
    }).where(and(eq(notasFiscais2.id, id), eq(notasFiscais2.userId, user().id))).returning();
    if (result.length === 0) {
      return c.json({ error: "Nota fiscal n\xE3o encontrada" }, 404);
    }
    return c.json({ data: result[0] });
  ...
  app.delete("/api/notas-fiscais/:id", async (c) => {
    ...
    await db.delete(itensNf2).where(eq(itensNf2.notaFiscalId, id));
    const result = await db.delete(notasFiscais2).where(and(eq(notasFiscais2.id, id), eq(notasFiscais2.userId, user().id))).returning();
    if (result.length === 0) {
      return c.json({ error: "Nota fiscal n\xE3o encontrada" }, 404);
    }
    return c.json({ success: true });
  ...
  app.get("/api/notas-fiscais/:notaId/itens", async (c) => {
    ...
    const result = await db.select().from(itensNf2).where(and(eq(itensNf2.notaFiscalId, notaId), eq(itensNf2.userId, user().id)));
    return c.json({ data: result });
  });
  app.post("/api/notas-fiscais/:notaId/itens", async (c) => {
    ...
    const result = await db.insert(itensNf2).values({
      notaFiscalId: notaId,
  ...
  app.put("/api/itens-nf/:id", async (c) => {
    ...
    const result = await db.update(itensNf2).set({
      descricao: body.descricao,
      categoria: body.categoria || null,
      quantidade: body.quantidade,
      unidade: body.unidade || "UN",
      precoUnitario: body.precoUnitario,
      desconto: body.desconto || 0
    }).where(and(eq(itensNf2.id, id), eq(itensNf2.userId, user().id))).returning();
    if (result.length === 0) {
      return c.json({ error: "Item n\xE3o encontrado" }, 404);
    }
    return c.json({ data: result[0] });
  });
  app.delete("/api/itens-nf/:id", async (c) => {
    ...
    const result = await db.delete(itensNf2).where(and(eq(itensNf2.id, id), eq(itensNf2.userId, user().id))).returning();
    if (result.length === 0) {
      return c.json({ error: "Item n\xE3o encontrado" }, 404);
    }
    return c.json({ success: true });
  ...
  app.get("/api/catalogo", async (c) => {
    ...
    const result = await db.select().from(itensCatalogo2).where(eq(itensCatalogo2.userId, user().id)).orderBy(desc(itensCatalogo2.createdAt));
    return c.json({ data: result });
  });
  app.post("/api/catalogo", async (c) => {
    const body = await c.req.json();
    console.log("[API] POST /api/catalogo - descricao:", body.descricao);
    const result = await db.insert(itensCatalogo2).values({
      descricao: body.descricao,
      categoria: body.categoria || null,
      unidade: body.unidade || "UN",
  ...
  app.post("/api/catalogo/import", async (c) => {
    ...
    if (!body.itens || !Array.isArray(body.itens)) {
      return c.json({ error: "Lista de itens inv\xE1lida" }, 400);
    }
    const values = body.itens.map((item) => ({
    ...
    const result = await db.insert(itensCatalogo2).values(values).returning();
  ...
  app.delete("/api/catalogo/all", async (c) => {
    ...
    const result = await db.delete(itensCatalogo2).where(eq(itensCatalogo2.userId, user().id)).returning();
  ...
  app.delete("/api/catalogo/:id", async (c) => {
    ...
    const result = await db.delete(itensCatalogo2).where(and(eq(itensCatalogo2.id, id), eq(itensCatalogo2.userId, user().id))).returning();
    if (result.length === 0) {
      return c.json({ error: "Item n\xE3o encontrado" }, 404);
    }
    return c.json({ success: true });
  ...
  app.get("/api/dashboard", async (c) => {
    ...
    const notas = await db.select().from(notasFiscais2).where(eq(notasFiscais2.userId, user().id)).orderBy(desc(notasFiscais2.dataEntrada));
    const todosItens = await db.select().from(itensNf2).where(eq(itensNf2.userId, user().id));
    let valorTotal = 0;
    let totalItens = 0;
    ...
    todosItens.forEach((item) => {
      ...
      const preco = Number(item.precoUnitario) || 0;
      const desc2 = Number(item.desconto) || 0;
      const valorItem = qtd * preco - desc2;
      valorTotal += valorItem;
      totalItens += 1;
      totalDescontos += desc2;
    ...
    const notasComValores = notas.map((nota) => {
      ...
      if (!porFornecedor[forn]) {
        porFornecedor[forn] = { quantidade: 0, valor: 0, notas: /* @__PURE__ */ new Set() };
      }
      ...
      itensNota.forEach((item) => {
        ...
        const descItem = item.descricao || "Sem descri\xE7\xE3o";
    ...
    const valorPorMes = Object.entries(porMes).map(([mes, valor]) => ({ mes, valor })).sort((a, b) => a.mes.localeCompare(b.mes));
    const itensPorCategoria = Object.entries(porCategoria).map(([categoria, data]) => ({
      categoria,
      quantidade: data.quantidade,
      valor: data.valor,
      percentual: valorTotal > 0 ? data.valor / valorTotal * 100 : 0
    })).sort((a, b) => b.valor - a.valor);
    const topItensList = Object.entries(topItens).map(([descricao, data]) => ({
      descricao,
      quantidade: data.quantidade,
      valor: data.valor
    })).sort((a, b) => b.valor - a.valor).slice(0, 10);
    const entradasPorFornecedor = Object.entries(porFornecedor).map(([fornecedor, data]) => ({
      fornecedor,
      quantidade: data.quantidade,
      valor: data.valor,
      notas: data.notas.size
    })).sort((a, b) => b.valor - a.valor).slice(0, 10);
    const gastosPorFilial = Object.entries(porFilial).map(([filial, data]) => ({
      filial,
      quantidade: data.quantidade,
      valor: data.valor,
      notas: data.notas,
